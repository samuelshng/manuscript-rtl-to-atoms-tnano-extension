...
When defining arithmetic operations in \gls{rtl} Verilog such as \texttt{product = w * a} or \texttt{product + partial\_sum\_in}, the exact choice of \glspl{alu} is left to subsequent synthesis steps to determine (further discussed in \secref{sec:methodology-eda}). In the absence of exact \gls{alu} specification, the default choices made by synthesis tools would typically opt for \gls{cmos}-optimized implementations, which can underperform in \gls{fcn} logic \cite{kim2010multipliers} due to different area and latency scaling that comes with the strictly planar technology where gates and wires must share the same plane.
To ensure the most optimal \glspl{alu} are chosen, this work explicitly defines the gate-level Verilog for a ripple-carry adder and an array multiplier---both previously found to be ideal for \gls{fcn} thanks to their relative simplicity which reduces wiring congestion and the need for crossovers\cite{kim2010multipliers}\todo{Awkward sentence, simplify}---and employ them in the \gls{pe}.
