...
Despite this progress, the conference implementation left several aspects only coarsely explored: the choice of arithmetic units was left to logic synthesis frameworks, which does not take \gls{fcn}'s unique architectural preferences into account; only the 8-bit weight and activation bit-widths were \hl{explored}\todo{reuse of word very soon}, \hl{leaving the scaling behavior of the {\gls{rtl}} unexplored}\todo{wording isn't clear, and also there may be a fundamentally better framing to put here---what did sweeping through more sizes enable this work to find out? Work from that and craft a narrative here to say what did lack thereof cause the original work to miss}; and \TODO{if Marcel \& Simon's work on adding 2i2o support in \emph{fiction} works out, mention that extra point}. This work therefore provides a substantially extended treatment of the \gls{mxu} \gls{pe}\todo{Don't want to introduce PE here, it's an implementation detail that should be introduced in later sections, here let's use a more generic wording}, introducing \gls{fcn}-appropriate arithmetic building blocks, generalized quantization settings, and \gls{fom}-guided, clocking-aware layout synthesis\todo{FoM-guided tech mapping was already present in the original conference paper.} atop the original \gls{rtl}-to-atoms flow. \TODO{One-liner aspirational statement that says what this work enables, to encapsulate the narrative of the intro and keep the reader motivated to continue reading this work.} The remainder of this manuscript is structured as follows. \secref{sec:background} reviews \gls{sidb} device operation and the relevant \gls{fcn} and \gls{eda} infrastructure; \secref{sec:related-work} summarizes prior \gls{sidb} \gls{mxu} designs; \secref{sec:methodology} details the hierarchical \gls{rtl} Verilog \hl{design}\todo{repetitive wording} and the \gls{rtl}-to-atoms synthesis flow; \secref{sec:results} presents the experimental protocol and synthesis results; and \secref{sec:conclusion-future-work} concludes with directions for future work.
...
\begin{figure}
    ...
    \subfloat[\TODO{Clocking}.]{
        \includegraphics[width=0.6\linewidth]{example-image-a}
        \label{sfig:sidb-logic-clocking}
    }
    \caption{
        \TODO{Paraphrase.}
...
The \gls{mac} operation is \hl{simply}\todo{Perhaps better wording is possible? It is indeed simple, almost trivial, it is mentioned just in case some readers aren't directly aware of MXU operations}:
%
\begin{equation}
