...
\begin{todolist}
    ...
    \item[\done] Extend Verilogs to accept arbitrary width and activation bit-width settings
    \item Add half-adder gate to technology mapper
    \begin{itemize}
        \item Simon to look into adding HA to TM and synthesizing via \emph{gold}
    \end{itemize}
    \item New table similar to DATE's table: report multiple algorithm combinations (e.g., ortho with and without PLO, FoM vs. uniform gates), test multiple bit-widths (W8A8, W4A4), report FoM-informed results similarly
\end{todolist}
...
\TODO{Mention that the bit width of weights and activations are adjustable.}
...
Starting with the \gls{rtl}-to-netlist stage, this work uses \emph{Yosys}~\cite{wolf2013yosys} to map arithmetic operators such as multiplication (\texttt{*}) and accumulation (\texttt{+}) into gate-level \gls{alu} structures.
...
The verified gate-level netlist is subsequently forwarded to \emph{fiction}~\cite{walter2019fiction}, whose \gls{fcn}-aware toolchain synthesizes it into an \gls{sidb} layout following the methodology in \cite{walter2022hexagons}:
%
\begin{enumerate}
  ...
  \item Placement and routing with the \textit{ortho} algorithm~\cite{walter2019scalable} produces a layout on a Cartesian grid.
