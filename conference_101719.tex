...
\begin{figure}
    ...
    \caption{
        A 2D systolic array \gls{mxu} receiving quantized activations ($a$), weights ($w$), weight control signals ($c_w$), and partial sums ($s$). \Glsentryshortpl{pe} inside the \gls{mxu} consist of a \gls{mac} unit, memory controller, and delay-line memory for weight storage. Signals propagate downward on the left side of the \glsentryshort{pe} (forward pass) and upwards on the right side (return pass).asdf
...
While that blueprint study reported promising area and power estimates against Google's \gls{tpu}v1~\cite{ng2023blueprint, jouppi2017indatacenter}, its extrapolated nature offered neither a manufacturable layout nor formal verification. The preceding conference version of this work~\cite{ng2025building} bridged this gap by applying\todo{Note to Marcel: to avoid overclaiming the conf paper's contribution, the wording claims that it \textit{applied} a synthesis flow rather than proposing it (since it didn't), please let me know whether this claim is clean enough; if there's still risk of being perceived as overclaiming I can further word smith this} latest \gls{sidb} \gls{eda} techniques to demonstrate a \gls{rtl}-to-atoms synthesis flow to a quantized \gls{sidb} \gls{mxu}. This was achieved through a hierarchical \gls{rtl} structure that satisfied \gls{eda} tooling requirements while enabling clocking-level validation. This journal extension substantially refines the flow by introducing technology-specific optimizations and exploring the scaling behavior of different quantization bit-widths, as detailed in the following section.
...
\secref{sec:methodology-verilog} describes the hierarchical \gls{rtl} architecture at the core of the synthesis flow satisfying tooling requirements.
...
\begin{enumerate}
  ...
  \item Placement and routing with the \textit{ortho}~\cite{walter2019scalable} or \gls{gold}~\cite{hofmann2024born,hofmann2025physical,hofmann2025graphoriented} algorithm produces a layout on a Cartesian grid.
  \item \Gls{plo}~\cite{hofmann2023postlayout,hofmann2024late,hofmann2025efficient} reduces the footprint of the routed design.
...
This rotation, however, can lead to suboptimal results: a layout that is compact on the Cartesian grid may become significantly taller after hexagonalization because diagonal signal paths in the Cartesian layout map to vertical paths in the hexagonal layout, especially when input pins are strongly offset in the Cartesian layout. Layouts with a more balanced aspect ratio are therefore preferred.
...
The hexagonalization algorithm was also revised in this work to align generated layouts with \gls{sidb} clocking expectations. Under the Cartesian layout's 2DDWave clocking scheme~\cite{vankamamidi2008twodimensional}, primary inputs are placed along the north and west borders and data advances diagonally toward the southeast. The $45^\circ$ rotation left many \gls{io} pins recessed from the north and south edges. These offsets reduce the attainable throughput because some signals must be held across multiple cycles for synchronization. The updated flow stretches all input and output pins to the north and south boundaries after projection, producing layouts whose ports are time-aligned as they enter and exit the hexagonal fabric. A configurable constraint can also be toggled to perform the pin extensions without wire crossings as they are typically less robust against environmental defects.
...
%       \TODO{Simon: Do we really need the "vs $A_{uni}$" column? I think we could also squeeze the table into one column without it then.}
%       \TODO{Simon: Should we also mention here that PLO was used together with each algorithm?}
%     \end{minipage}
%   \end{minipage}
% \end{table*}
  ...
  \begin{minipage}{\linewidth}
    \centering
      ...
      \begin{minipage}{\linewidth}
        \footnotesize
        \vspace{0.5em}
        $A_\mathrm{min}$ and $\bar{A}$ represent the minimum and mean areas.
        Uniform technology mapping was used.
        The percentage differences compare synthesized areas against the ``area'' cost objective.
        All trials ran \gls{gold} in single-core mode with a timeout of \SI{400}{\second} on an AMD Ryzen 9 9950X3D with maximum effort mode, input pin tile skipping was swept from \numrange{1}{6} with randomization. $630$ trials were performed per configuration, with unsuccessful results discarded.
...
This comparative study characterizes how activation and weight bit-widths scale and reports synthesis results across different technology-mapping and placement-and-routing settings.
...
This work has demonstrated an end-to-end \gls{rtl}-to-atoms synthesis flow for a quantized \gls{sidb}-based \gls{mxu} \gls{pe} with fully verifiable behavior across abstraction levels, enabled by a hierarchical \gls{rtl} organization that separates a combinational \gls{pe}-core from its clocked shell. By explicitly selecting \gls{sidb}-appropriate \glspl{alu} in \emph{Yosys} and tailoring technology mapping, placement-and-routing, and hexagonalization to \gls{sidb} logic, the presented framework achieves roughly \SI{15}{\percent} reduction in synthesized \gls{pe}-core area over prior flows and realizes additional footprint savings of $\approx\!40\text{--}50\,\%$ for configurations within the scaling range of newer placement-and-routing algorithms, while making explicit the trade-off between footprint and device-level robustness under \gls{fom}-aware mapping. Together, these components establish a reproducible benchmark for atomic-scale \gls{eda} that links \gls{rtl} design decisions, \gls{eda} tooling, and manufacturable \gls{sidb} layouts. %, and clarify the remaining \mbox{$3.4\times$} area gap relative to hand-crafted blueprint estimates.

% The results of this study demonstrate that a carefully engineered \gls{rtl}-to-atoms flow can realize a quantized \gls{sidb}-based \gls{mxu} \gls{pe} with fully verifiable behavior across abstraction levels. By adopting a hierarchical \gls{rtl} organization that separates the combinational \gls{pe}-core from its clocked shell, the presented framework satisfies the purely combinational requirements of \emph{fiction} while retaining cycle-accurate verification at the architectural level. Combined with explicitly specified \glspl{alu} optimized for \gls{sidb} logic in \emph{Yosys}, this organization enables technology-appropriate arithmetic implementations that reduce the synthesized \gls{pe}-core area by approximately \SI{15}{\percent} relative to prior flows, while parameterization over multiple weight and activation precisions exposes how quantization choices affect both layout size and the feasibility of advanced placement-and-routing algorithms. The integration of \gls{fom}-aware technology mapping, refined \gls{gold} cost objectives, and improved pin handling in hexagonalization further tailors the netlist-to-atoms flow to \gls{sidb} logic, achieving additional footprint reductions of $\approx\!40\text{--}50\,\%$ relative to \emph{ortho}-based baselines for configurations within \gls{gold}'s scaling range. At the same time, \gls{fom}-aware mapping prioritizes physically robust gates even when this increases area, making explicit the trade-off between footprint and device-level reliability. Together, these components establish a reproducible benchmark for atomic-scale \gls{cad} that links algorithmic design decisions, \gls{eda} tool settings, and manufacturable \gls{sidb} layouts, while a comparison against earlier blueprint estimates highlights both the progress made and the remaining \mbox{$3.4\times$} area gap to hand-crafted designs.

The findings also reveal several avenues for future work to bridge remaining gaps with manual expert designs.
Preservation of arithmetic structures (e.g., adders and multipliers) across the synthesis stages will allow optimized technology-specific \gls{alu} layouts to be exploited more directly in the physical layout stage.
On physical design, improving the scalability and robustness of \gls{gold} and \gls{plo} to handle larger scale netlists accelerators could enable the demonstrated footprint advantages to carry over to W8A8-scale layouts and larger accelerators.
Finally, automating full \gls{pe}- and \gls{mxu}-level synthesis---including routing of the return path and of the pins between the \gls{pe}-core and the outer \gls{pe} module \gls{io}---will enable application-scale studies that jointly consider device reliability, clocking constraints, and workload characteristics in \gls{sidb}-based computing systems. Taken together, this work's successful demonstration of an end-to-end \gls{rtl}-to-atoms flow for a quantized \gls{sidb}-based \gls{mxu} \gls{pe} provides a concrete methodological foundation for these advances and is expected to help close the gap between synthesized layouts and expert designs in atomic-scale computing.
