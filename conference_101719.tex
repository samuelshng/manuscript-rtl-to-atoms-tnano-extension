...
\begin{figure}
    ...
    \caption{
        A 2D systolic array \gls{mxu} receiving quantized activations ($a$), weights ($w$), weight control signals ($c_w$), and partial sums ($s$). \Glspl{pe} inside the \gls{mxu} consist of a \gls{mac} unit, memory controller, and delay-line memory for weight storage. Signals propagate downward on the left side of the \gls{pe} (forward pass) and upwards on the right side (return pass).asdf
...
%
\begin{description}
  \item[Combinational core:] The \gls{pe}'s forward-pass logic is implemented in strictly combinational \gls{rtl}, as shown in Algorithm~\ref{alg:pe-forward}, ensuring compatibility with subsequent synthesis steps.
...
The findings also reveal several avenues for future work.
Although \emph{Yosys} can be instructed to use \gls{sidb}-optimized \glspl{alu} when synthesizing \gls{rtl} to gate-level netlists, the boundaries between individual \glspl{alu} and other logic components blur in subsequent optimization steps, preventing the direct use of predesigned layouts for common arithmetic blocks optimized for the technology. Improvements to synthesis workflows that preserve these reusable optimal components could substantially reduce the area cost of ubiquitous \glspl{alu}. On the physical-design side, improving the scalability and robustness of \gls{gold} and \gls{plo}---for example, scaling reliably to W8A8-scale netlists---would enable the demonstrated footprint advantages to carry over to higher precisions and larger accelerators. Finally, automating full \gls{pe}-level synthesis, including routing of the return path and of the pins between the \gls{pe}-core and the outer \gls{pe} module \gls{io}, and expanding the flow to complete \gls{mxu} systolic arrays would permit application-scale studies that jointly consider device reliability, clocking constraints, and workload characteristics. Taken together, this work's successful demonstration of an end-to-end \gls{rtl}-to-atoms flow for a quantized \gls{sidb}-based \gls{mxu} \gls{pe} provides a concrete methodological foundation for these advances and is expected to help close the gap between synthesized layouts and expert designs in atomic-scale computing.
