...
\subsection{Electronic Design Automation for FCN}
...
\subsection{SiDB ML Accelerators}
...
Finally, the resulting \gls{aig} is validated against the \gls{pe}-core test benches to confirm functional correctness.
...
Comparing placement and routing algorithms, while the W8A8 \gls{pe} has an \gls{aig} that exceeds \gls{gold}'s tractable problem size, W4A4 and W2A2 remain sufficiently small for \gls{gold} to successfully place and route layouts, achieving a $\approx \SIrange{40}{50}{\percent}$ area reduction relative to their \emph{ortho}-based counterparts. In the \gls{fom}-aware results, however, a pronounced area increase arises for W4A4, where the \gls{gold} result is even larger than that from \emph{ortho}. This work interprets the outlier as evidence that, under \gls{fom}-aware technology mapping, the \gls{aig} is already near the upper bounds of \gls{gold}'s ability to provide consistent solutions: among $630$ trials for this configuration, only $1$ reached a completed layout. Nevertheless, for configurations that remain within \gls{gold}'s effective scaling regime, it delivers area reductions that directly improve manufacturability and reduce power consumption in practice.
