...
\end{equation}
%
with $s_\text{in}$ the input partial sum computed by the preceding \gls{pe}, $a$ the activation, $w$ the weight, and $s_\text{out}$ the output partial sum.
...
At the \gls{pe} granularity, each \gls{pe} is split into two signal paths: a \emph{forward-pass} which contains the \gls{mac} unit and memory controller; and a \emph{return-pass} which carries a weight bus upwards to complete the feedback loop for a delay-line memory, and an activation bus to synchronize the activations with the neighboring \gls{pe}. This arrangement, illustrated in \fref{sfig:schem_pe}, allows each pass to be purely combinational, which allows them to employ row-wise clocking\todo{improve awkward wording}.
\OLD{The implementation is deeply pipelined, a trait that is common among \gls{fcn} technologies in consideration of operational reliability and in order to facilitate signal propagation. This means that, at each clock cycle, different inputs can be interleaved to increase the computational throughput of the \gls{mxu}.
...
\TODO{Describe shortcomings that we've identified in the conference paper, briefly explain architecturally (e.g., cost of crossings) why alternatives are better, and then clearly state our contributions (explicitly defined N-bit ripple-carry adder and array multiplier generators) to close the gap. See if we can generate some figures for this. Another thing is to see whether we can deduce which adder/multiplier implementation Yosys went for.}
