...
%
\begin{description}
  \item[Combinational core:] The \gls{pe}'s forward-pass logic is implemented in strictly combinational \gls{rtl}, as shown in Alg.~\ref{alg:pe-forward},\TODO{Simon: shown in Algorithm instead of Alg.?} ensuring compatibility with subsequent synthesis steps.
...
The verified gate-level netlist is subsequently forwarded to \emph{fiction}~\cite{walter2019fiction} \TODO{Simon: I don't think we have to cite fiction that often}, whose \gls{fcn}-aware toolchain synthesizes it into an \gls{sidb} layout following the synthesis flow proposed by \cite{walter2022hexagons}:
%
\begin{enumerate}
  ...
  \item \Gls{plo}~\cite{hofmann2023postlayout, hofmann2024late} reduces the footprint of the routed design. \TODO{Simon: Post-layout optimization instead of Post-layout Optimization? Can we also cite the TCAD Journal version? https://ieeexplore.ieee.org/document/10916761}
