...
\begin{figure}
    ...
    \caption{
        ...
        \TODO{Idea for further improvement: add an additioanl panel underneath the SA and PE which ``zooms'' into the forward pass and shows 1. a clocked abstraction of pipeline stages (possibly with Verilogs?) and 2. a few lines of Verilog showcasing the key logic, and then an arrow pointing to Yosys/ABC/fiction, then an arrow pointing to a zoomed in snapshot of a synthesized layout (similar to BPN paper figure 2 showcasing a zoomed-in view of a synthesized layout with the edges faded out. A potential alternative way to realize this figure is by making a full-width horizontal fig.)}
    }
...
\end{enumerate}
%
While this established workflow provides a robust foundation, its application to the \gls{sidb} platform revealed several opportunities for technology-specific enhancements, which are detailed below.
...
This work adopts $\chi$ as the objective for the technology mapper so that the flow prioritizes gates with higher predicted robustness, accepting a potential trade-off in other layout metrics like area or device count. The resulting trade-offs are examined in \secref{sec:results}.
...
While the older \emph{ortho} algorithm offers no configuration to compensate for this, the more recent \gls{gold} algorithm~\cite{hofmann2024born} exposes a configurable cost function for placement and routing, enabling optimizations more suitable for the final \gls{sidb} layout.

\TODO{Mention what the default cost function was, introduce the proposed cost function that is more suitable of SiDBs and explain why.}
